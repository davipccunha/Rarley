module.exports = {
    ...
    async execute(client, Discord, config, oldMessage, newMessage) {
        ...
        // Logs
        const server = config.guilds.rarley.id;

        if (!oldMessage.author) return;

        if (oldMessage.author.bot || oldMessage.channel.type == 'dm') return;

        const channel = oldMessage.guild.id == config.guilds.rarley.id ? client.guilds.resolve(server).channels.resolve(config.guilds.rarley.channels.logs) : client.guilds.resolve(config.guilds.equipe.id).channels.resolve(config.guilds.equipe.channels.logs);
            ...
            if (!oldMessage.content) {
                if (newMessage.content.length > 1024) {
                    channel.send(new Discord.MessageEmbed()
                        ...
                        .setTitle('| Logs • Rarley ▸ Mensagem editada')
