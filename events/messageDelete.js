module.exports = {
    ...
    async execute(client, Discord, config, message) {
        ...
        const channel = message.guild.id == config.guilds.rarley.id ? client.guilds.resolve(server).channels.resolve(config.guilds.rarley.channels.logs) : client.guilds.resolve(config.guilds.equipe.id).channels.resolve(config.guilds.equipe.channels.logs);

        if (message.attachments.first()) {
            if (!message.content) {
                channel.send(new Discord.MessageEmbed()
                    .setColor(client.guilds.resolve(server).member(client.user).displayHexColor)
                    .setTitle('| Logs • Rarley ▸ Mensagem deletada')
